package com.example.app21try6.database.daos

import androidx.lifecycle.LiveData
import androidx.room.Dao
import androidx.room.Insert
import androidx.room.OnConflictStrategy
import androidx.room.Query
import androidx.room.Transaction
import androidx.room.Update
import com.example.app21try6.database.models.SubWithPriceModel
import com.example.app21try6.database.tables.Product
import com.example.app21try6.database.tables.SubProduct

@Dao
interface SubProductDao {
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    fun insert(subProduct: SubProduct)
    @Update
    fun update(subProduct: SubProduct)

    @Query("DELETE FROM sub_table WHERE sPCloudId= :id_")
    fun delete(id_:Long)

    @Query("UPDATE sub_table SET is_checked =:bool  WHERE sub_name =:name")
    fun update_checkbox(name:String,bool:Int):Int
    @Query("UPDATE sub_table SET is_checked =0")
    fun unchecked_allCheckbox()
    @Query("SELECT * FROM sub_table s " +
            "JOIN product_table p  ON s.productCloudId = p.productCloudId "+
            "WHERE  (:productCloudId_ IS NULL OR s.productCloudId = :productCloudId_)" +
            " AND (:brandId IS NULL OR p.brand_code=:brandId)")
    fun getAll(productCloudId_:Long?,brandId: Long?):LiveData<List<SubProduct>>



    @Query("""
        SELECT s.sub_name FROM sub_table s
        JOIN product_table p  ON s.productCloudId = p.productCloudId
        WHERE p.product_name = :name
        ORDER BY s.sub_name
    """)
    fun getSpNameListByProductName(name:String):List<String>

    @Query("SELECT productCloudId FROM sub_table WHERE sPCloudId = :sub_id_")
    fun getProductIdBySubId(sub_id_:Long?):Long?

    @Query("SELECT * FROM sub_table WHERE sPCloudId = :sub_id_")
    fun getSubProductBySubId(sub_id_:Long?): SubProduct

    @Query("SELECT sPCloudId FROM sub_table WHERE sub_name=:subName")
    fun getSubIdBySubName(subName:String):Long

    @Query("SELECT sub_name FROM sub_table WHERE sPCloudId = :sub_id_")
    fun getNama(sub_id_: Long):LiveData<String>

    @Query("SELECT warna FROM sub_table WHERE sPCloudId = :sub_id_")
    fun getWarna(sub_id_: Long):LiveData<String>

    @Query("SELECT ket FROM sub_table WHERE sPCloudId = :sub_id_")
    fun getKet(sub_id_: Long):LiveData<String>
    @Query("SELECT roll_u FROM sub_table WHERE sPCloudId = :sub_id_")
    fun getStokU(sub_id_: Long):LiveData<Int>

    @Query("INSERT INTO sub_table (sub_name,roll_u,warna,ket,is_checked,productCloudId) SELECT :sub_name_ as sub_name,:u_ as roll_u, :warna_ as warna,:ket_ as ket,0 as is_checked,(SELECT productCloudId FROM product_table WHERE product_name = :product_name_ LIMIT 1) as productCloudId")
    fun insertIfNotExist(sub_name_:String,warna_:String,ket_:String,u_:Int,product_name_: String)


    @Query("SELECT *FROM sub_table INNER JOIN PRODUCT_TABLE ON  product_table.productCloudId = sub_table.productCloudId WHERE sPCloudId = :id limit 1")
    fun getProduct(id: Long): Product?



    @Query("SELECT sp.*, p.purchasePrice, p.default_net " +
            "FROM sub_table sp " +
            "INNER JOIN product_table p ON sp.productCloudId = p.productCloudId " +
            "WHERE sp.sub_name  LIKE '%' || :query || '%' " +
            "LIMIT 20")
    fun getSubProductWithPrice(query: String):LiveData<List<SubWithPriceModel>?>


    @Query("""
        UPDATE trans_detail_table
        SET trans_item_name =:updatedName, needs_syncs=1
        WHERE sPCloudId =:sub_id
    """)
    suspend fun updateTransItemName(sub_id: Long, updatedName:String)

    @Transaction
    suspend fun updateSubProductAndTransDetail(
        subProduct: SubProduct
    ) {
        update(subProduct)
        updateTransItemName(subProduct.sPCloudId, subProduct.sub_name)
    }
    @Query("""
        UPDATE sub_table
        SET sPCloudId =:cloudId
        WHERE sPCloudId=:id
    """)
    fun assignSubProductCloudID(cloudId:Long,id: Long)

    @Query("SELECT * FROM sub_table ")
    fun selectAllSubProductTable(): List<SubProduct>
    @Query("""
    SELECT *
    FROM sub_table
    WHERE sPCloudId IN (
        SELECT sPCloudId
        FROM sub_table
        GROUP BY sPCloudId
        HAVING COUNT(sPCloudId) > 1
    )
    ORDER BY sPCloudId
""")
    fun getSubProductsWithDuplicateCloudIds(): List<SubProduct>

    @Query("UPDATE sub_table SET needs_syncs = 0 WHERE sPCloudId = :cloudId")
    suspend fun markAsSynced(cloudId: Long)
}